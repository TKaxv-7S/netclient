...
func (app *App) GoPullLatestNodeConfig(network string) (Network, error) {
	var net Network
	response, err := httpclient.GetResponse("", http.MethodGet, url+"/pull/"+network, "", headers)
	if err != nil {
		return net, err
	}
	if response.StatusCode != http.StatusOK {
		return net, fmt.Errorf("http status err %d %s", response.StatusCode, response.Status)
	}
	if err := json.NewDecoder(response.Body).Decode(&net); err != nil {
		return net, err
	}
	return net, nil
}
